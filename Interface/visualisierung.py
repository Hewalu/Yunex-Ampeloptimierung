...
def load_images():
    ...
    if not os.path.exists(asset_dir): sys.exit()
...
def draw_led_ring(screen, active_leds, total_leds, state, breathing_alpha=255):
    ...
    if total_leds > 60: current_dot_size = max(2, int(DOT_SIZE_BASE * (60/total_leds)))
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_UP: person_count = min(MAX_PERSON_CAP, person_count + 1)
                if event.key == pygame.K_DOWN: person_count = max(0, person_count - 1)
        ...
        # --- ROT PHASE (WARTEN) ---
            ...
            if timer_elapsed >= timer_total_duration_red:
                ...
                if green_leds_left_float > MAX_LEDS_LIMIT: green_leds_left_float = float(MAX_LEDS_LIMIT)
                slow_mode_active = False
