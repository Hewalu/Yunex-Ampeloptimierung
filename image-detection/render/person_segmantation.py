...
def get_id_color(id):
    """Generiert eine eindeutige Farbe basierend auf der Track-ID."""
    ...
    while cap.isOpened():
        ...
        if results[0].boxes.id is not None and results[0].masks is not None:
            ...
            for i, track_id in enumerate(track_ids):
                ...
                # 1. Maske zeichnen (wenn vorhanden)
                if i < len(masks):
                    polygon = masks[i]
                    if len(polygon) > 0:
                ...
                # 2. Pfad (Trail) aktualisieren und zeichnen
                ...
                cv2.polylines(annotated_frame, [points], isClosed=False, color=color, thickness=4)

            # Overlay mit Transparenz anwenden
            alpha = 0.5
            cv2.addWeighted(overlay, alpha, annotated_frame, 1 - alpha, 0, annotated_frame)

        # Fallback falls keine Masken da sind, aber Boxen
